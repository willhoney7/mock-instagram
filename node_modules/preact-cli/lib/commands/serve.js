...
let serve = (() => {
	var _ref = _asyncToGenerator(function* (options) {
		let dir = _path2.default.resolve(options.cwd || process.cwd(), options.dir || '.');
	...
	config({ configObj, options }) {
		return _asyncToGenerator(function* () {
			...
			if (options.dest && options.dest !== '-') {
				let isDir = false;
				try {
					isDir = (yield _fs2.default.stat(options.dest)).isDirectory();
				} catch (e) {}
				if (isDir) {
					dir = options.dest;
...
const tmpFile = opts => new Promise((resolve, reject) => {
	_tmp2.default.file(opts, (err, path) => {
